...
# Run with WandB integration - Smaller subset for quick experimentation
python -u main.py \
  --data_path "${PWD}/Sentiment140.csv" \
  --output_dir model_output/V5_subset \
  --vocab_size 20000 \
  --emb_dim 128 \
  --stack_depth 3 \
  --attn_heads 4 \
