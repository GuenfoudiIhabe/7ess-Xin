#!/bin/bash
#SBATCH --job-name=SA_extraDeepV7
#SBATCH --nodes=1                    
...
# Run with WandB integration - Testing extra deep architecture (8 layers)
python -u main.py \
  --data_path "${PWD}/Sentiment140.csv" \
  --output_dir model_output/V7_extra_deep \
  --vocab_size 35000 \
  --emb_dim 128 \
  --stack_depth 8 \
  --attn_heads 4 \
  ...
  --wandb_name V7_extra_deep_model \
