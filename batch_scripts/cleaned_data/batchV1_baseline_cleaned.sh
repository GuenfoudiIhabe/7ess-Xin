#!/bin/bash
#SBATCH --job-name=SA_baseline_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Baseline model with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V1_baseline_cleaned \
  --vocab_size 30000 \
  --emb_dim 128 \
  --stack_depth 3 \
  --attn_heads 4 \
  ...
  --wandb_name V1_baseline_cleaned \
