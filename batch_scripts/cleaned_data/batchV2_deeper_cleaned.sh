#!/bin/bash
#SBATCH --job-name=SA_deeper_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Deeper architecture with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V2_deeper_cleaned \
  --vocab_size 30000 \
  --emb_dim 128 \
  --stack_depth 6 \
  --attn_heads 4 \
