#!/bin/bash
#SBATCH --job-name=SA_tuned_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Hyperparameter tuned model with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V4_tuned_cleaned \
  --vocab_size 30000 \
  --emb_dim 128 \
  --stack_depth 3 \
  --attn_heads 4 \
