#!/bin/bash
#SBATCH --job-name=SA_subset_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Smaller subset with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V5_subset_cleaned \
  --vocab_size 20000 \
  --emb_dim 128 \
  --stack_depth 3 \
  --attn_heads 4 \
