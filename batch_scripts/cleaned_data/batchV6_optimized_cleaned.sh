#!/bin/bash
#SBATCH --job-name=SA_optimized_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Optimized model with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V6_optimized_cleaned \
  --vocab_size 35000 \
  --emb_dim 128 \
  --stack_depth 6 \
  --attn_heads 4 \
  ...
  --wandb_name V6_optimized_cleaned \
