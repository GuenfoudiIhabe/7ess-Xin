#!/bin/bash
#SBATCH --job-name=SA_extraDeep_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Extra deep architecture with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V7_extra_deep_cleaned \
  --vocab_size 35000 \
  --emb_dim 128 \
  --stack_depth 8 \
  --attn_heads 4 \
  ...
  --wandb_name V7_extra_deep_cleaned \
