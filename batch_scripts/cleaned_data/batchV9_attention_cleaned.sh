#!/bin/bash
#SBATCH --job-name=SA_attention_cleaned
#SBATCH --nodes=1                    
...
# Run with WandB integration - Focused on attention mechanism with cleaned data
python -u main.py \
  --data_path "${PWD}/cleaned_sentiment_data.csv" \
  --output_dir model_output/V9_attention_cleaned \
  --vocab_size 35000 \
  --emb_dim 128 \
  --stack_depth 6 \
  --attn_heads 16 \
  ...
  --wandb_name V9_attention_cleaned \
